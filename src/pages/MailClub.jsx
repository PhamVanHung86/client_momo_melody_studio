...
// import {
//   setPendingMailClubPlan,
//   consumePendingMailClubPlan,
// } from "../utils/pendingMailClubPlan";
...
//       const pendingPlan = consumePendingMailClubPlan();
//       if (pendingPlan) {
...
//     try {
//       const res = await fetch(apiUrl("/api/mail-club-collections"));
//       const data = await res.json();
//       if (data.success) {
...
//     if (!selectedPlan || !isFormOpen) return;

//     if (!user) {
//       setPendingMailClubPlan(selectedPlan);
//       navigate("/login", {
//         state: { from: location.pathname, intent: "register" },
//       });
//       return;
//     }
...
//           {loadingTheme ? (
//             // 💗 Khung giữ chỗ đúng hình dạng section thật (poster + hàng
//             // item) trong lúc chờ API trả về — tránh việc cả khối to đột
//             // ngột "đập" vào trang gây giật khi fetch xong.
//             <div className="relative bg-gradient-to-br from-[#8B98e3]/40 to-[#E8E4F5]/40 rounded-[2rem] px-4 py-8 md:p-10 border border-[#FFD6E0]/60 overflow-hidden animate-pulse">
//               <div className="flex flex-col items-center gap-3 mb-8">
//                 <div className="h-5 w-40 bg-white/50 rounded-full" />
//                 <div className="h-9 w-56 bg-white/60 rounded-full" />
//               </div>
//               <div className="flex flex-col gap-8 items-center">
//                 <div className="w-full max-w-[360px] aspect-[3/4] rounded-2xl bg-white/50" />
//                 <div className="flex gap-4 md:gap-6 justify-center w-full">
//                   {[...Array(4)].map((_, i) => (
//                     <div
//                       key={i}
//                       className="flex-none w-[170px] aspect-square rounded-xl bg-white/50"
//                     />
//                   ))}
//                 </div>
//               </div>
...
//                 className="relative bg-gradient-to-br from-[#8B98e3] to-[#E8E4F5] rounded-[2rem] px-4 py-8 md:p-10 border border-[#FFD6E0] shadow-[0_15px_40px_rgba(255,183,197,0.12)] overflow-hidden"
//               >
...
//                   disabled={!selectedPlan || !isFormOpen}
...
//                       : "Đăng ký ngay 🌸"}
//                 </button>
//                 {!user && selectedPlan && isFormOpen && (
//                   <p className="text-xs text-[#4A4A6A]/50 mt-3">
//                     Bạn cần có tài khoản để theo dõi gói Mail Club của mình —
...
//                 >
//                   Cùng xem sản phẩm các tháng trước
//                 </h2>
//                 <p className="text-xs text-[#4A4A6A]/50 mt-1">
//                   Xem lại những chủ đề mà mình đã vẽ trong các tháng trước đó
//                 </p>
//               </div>
...
const MailClub = () => {
  ...
  // Tìm thông tin gói đang chọn từ constants PLANS
      ...
      if (data.success) {
        ...
        if (data.collections.length > 0) {
          ...
          setCurrentTheme({
            ...
            menuImage: latestCollection.images[0] || "",
            productImages: latestCollection.images.slice(1, 5) || [],
            isOpen: latestCollection.isOpen !== false,
            openTime: latestCollection.openTime || null,
            closeTime: latestCollection.closeTime || null,
          });
        }
      }
    } catch (err) {
      ...
      <div className="bg-[#FFFAF5]">
          ...
          {!mySubscription && (
            <div>
              <div className="text-center mb-8 md:mb-10">
                ...
                <p className="text-xs md:text-sm text-[#4A4A6A]/50 mt-1.5">
                  Chọn gói phù hợp và nhận Mail club quà mỗi tháng 🎁
                </p>
              </div>

              {/* Hiển thị 2 cột (grid-cols-2) trên cả mobile và desktop */}
              <div className="grid grid-cols-2 gap-3 sm:gap-6 max-w-2xl mx-auto mb-8 md:mb-10">
                {PLANS.map((plan) => (
                  <div
                    key={plan.id}
                    onClick={() => isFormOpen && setSelectedPlan(plan.id)}
                    className={`relative rounded-2xl sm:rounded-3xl p-3.5 sm:p-6 transition-all duration-300 border-2 ${
                      !isFormOpen
                        ? "opacity-50 cursor-not-allowed border-transparent bg-gray-100"
                        : selectedPlan === plan.id
                          ? `${plan.border} ${plan.bg} scale-[1.02] shadow-lg cursor-pointer`
                  ...
                  >
                    {plan.recommended && isFormOpen && (
                      <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#8B98E3] text-white text-[10px] sm:text-xs font-bold px-2.5 sm:px-4 py-0.5 sm:py-1 rounded-full whitespace-nowrap shadow-xs">
                        Tiết kiệm hơn ⭐
                      </span>
                    )}
                    <div className="text-center mb-3 sm:mb-5">
                      <span className="text-2xl sm:text-4xl">{plan.emoji}</span>
                      <h3 className="text-sm sm:text-lg font-semibold text-[#4A4A6A] mt-1 sm:mt-2">
                        {plan.label}
                      </h3>
                      <p className="text-lg sm:text-2xl font-bold text-[#FFB7C5] mt-0.5 sm:mt-1">
                        {plan.price}
                      </p>
                      <p className="text-[10px] sm:text-xs text-[#4A4A6A]/50">
                        /{plan.duration}
                      </p>
                    </div>
                    <ul className="flex flex-col gap-1.5 sm:gap-2">
                      {plan.benefits.map((b, i) => (
                        <li
                          key={i}
                          className="flex items-start gap-1.5 sm:gap-2 text-xs sm:text-sm text-[#4A4A6A]/70"
                        >
                          <span className="text-[#FFB7C5] mt-0.5 shrink-0">
                            ✓
                          </span>
              ...
              <div className="text-center">
                <button
                  onClick={handleSelectPlanClick}
                  disabled={!selectedPlan || !isFormOpen}
                  className="bg-[#FFB7C5] text-white px-8 sm:px-12 py-3.5 sm:py-4 rounded-full text-xs sm:text-sm font-semibold hover:bg-[#ff9db5] transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-md"
                >
                  {!isFormOpen
                    ? "Cổng đăng ký đang đóng 🔒"
                    : !user && selectedPlan
        ...
        {showForm && !submitted && (
          <>
            ...
            <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md px-4">
              <div className="bg-white rounded-3xl p-6 shadow-2xl">
                {/* Header Popup có tên gói + đơn giá */}
                <div className="flex items-center justify-between border-b border-[#FFD6E0]/50 pb-3 mb-4">
                  <div>
                    <h3
                      style={{ fontFamily: "'Dancing Script', cursive" }}
                      className="text-2xl text-[#4A4A6A]"
                    >
                      {currentPlanObj?.label ||
                  ...
                  {currentPlanObj && (
                    <div className="text-right bg-[#FFF0F5] px-3 py-1.5 rounded-xl border border-[#FFD6E0]">
                      <span className="text-base font-bold text-[#FF8A9F]">
                        {currentPlanObj.price}
                      </span>
                      <p className="text-[10px] text-[#4A4A6A]/60">
        ...
        {submitted && (
          <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center px-4">
            <div className="bg-white rounded-3xl p-8 max-w-md w-full text-center shadow-2xl">
              ...
              {/* Khung thông tin ngân hàng + Thông tin gói & Đơn giá */}
              <div className="bg-[#FFF0F5] rounded-2xl p-4 text-left text-sm text-[#4A4A6A] mb-5 space-y-1.5">
                {currentPlanObj && (
                  <div className="bg-white/80 p-3 rounded-xl border border-[#FFD6E0] mb-3">
                    <p className="text-xs text-[#4A4A6A]/70">Gói đã chọn:</p>
                    <div className="flex items-center justify-between mt-0.5">
                      <span className="font-bold text-[#4A4A6A]">
                        {currentPlanObj.label}
                      </span>
                      <span className="text-base font-extrabold text-[#FF8A9F]">
                        {currentPlanObj.price}
                      </span>
                    </div>
                  </div>
                ...
                <p className="font-semibold text-[#4A4A6A] mb-1">
                ...
                <p className="text-[#FF8A9F] font-semibold mt-2 pt-1 border-t border-dashed border-[#FFD6E0]">
                  Nội dung CK: TÊN - SĐT
                </p>
              </div>
              ...
              <button
                ...
                className="w-full py-3 rounded-2xl bg-[#FFB7C5] text-white text-sm font-semibold mt-4"
              >
                Đóng
              </button>
            </div>
